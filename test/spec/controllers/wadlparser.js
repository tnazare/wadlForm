describe('WadlParser controller tests', function() {
  ...
  describe('Controller: WadlParser', function() {
    ...
    it('should parse xml response and extract entry infos' , function () {
        ...
        expect(entry.title.innerText).toBe("BEDELIX 3 g pdre p susp buv");
        ...
        expect(entry.id.innerText).toBe("vidal://product/1971");
        expect(entry.updated.innerText).toBe("2014-09-17T22:00:00Z");
        expect(entry.summary.type).toBe("text");
        expect(entry.summary.innerText).toBe("BEDELIX 3 g pdre p susp buv");
        expect(entry.vidal.id.innerText).toBe("1971");
        expect(entry.vidal.dispensationplace.name).toBe("PHARMACY");
        expect(entry.vidal.dispensationplace.innerText).toBe("PHARMACY");
        expect(entry.vidal.activeprinciples.innerText).toBe("montmorillonite beidellitique");
        expect(entry.vidal.horsghs.innerText).toBe("false");
        expect(entry.vidal.refundrate.name).toBe("NR");
        expect(entry.vidal.refundrate.innerText).toBe("NR");
        expect(entry.vidal.company.type).toBe("OWNER");
        expect(entry.vidal.company.vidalid).toBe("2908");
        expect(entry.vidal.company.innerText).toBe("Ipsen Pharma");
    });
